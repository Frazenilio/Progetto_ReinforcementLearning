...
BUFFER_SIZE = 5  # Dimensione del replay buffer
...
class ReplayBuffer:
    ...
    Memorizza le ultime BUFFER_SIZE transizioni (S_t, A_t, R_{t+1}, S_{t+1}, done)
    e le restituisce tutte come batch per il calcolo della loss.
    """
    ...
    def push(self, state, action, reward, next_state, done):
        ...
        Args:
            state:      tensor (1, 3, 210, 160) — stato corrente preprocessato
            action:     int — azione eseguita
            reward:     float — reward ricevuto
            next_state: tensor (1, 3, 210, 160) — stato successivo preprocessato
    ...
    def sample_all(self):
        """
        Restituisce tutte le transizioni nel buffer come batch di tensori.
        
        Returns:
            states:      tensor (N, 3, 210, 160)
            actions:     tensor (N,) di tipo long
            rewards:     tensor (N,)
            next_states: tensor (N, 3, 210, 160)
            dones:       tensor (N,)
        """
        batch = list(self.buffer)
        states, actions, rewards, next_states, dones = zip(*batch)

        states = torch.cat(states, dim=0)           # (N, 3, 210, 160)
        next_states = torch.cat(next_states, dim=0) # (N, 3, 210, 160)
        actions = torch.tensor(actions, dtype=torch.long)    # (N,)
        rewards = torch.tensor(rewards, dtype=torch.float32) # (N,)
        dones = torch.tensor(dones, dtype=torch.float32)     # (N,)
...
class FreewayCNN(nn.Module):
    ...
    def compute_loss_batch(self, states, actions, rewards, next_states, dones, gamma):
        """
        Loss DQN con Experience Replay (batch).
        
        L(w) = 1/N * Σ_i 1/2 * (R_i + γ * max_a q_w(S'_i, a) - q_w(S_i, A_i))^2
        
        Il target (R + γ * max Q') è detachato dal grafo: semi-gradient update.
        
        Args:
            states:      tensor (N, 3, 210, 160) — batch di stati
            actions:     tensor (N,) long — batch di azioni
            rewards:     tensor (N,) — batch di reward
            next_states: tensor (N, 3, 210, 160) — batch di stati successivi
            dones:       tensor (N,) — batch di flag done
            gamma:       float — fattore di sconto
        
        Returns:
            loss: tensor scalare (media sul batch)
        """
        # Forward pass sul batch di stati → Q-values (N, 3)
        q_values = self.forward(states)  # (N, 3)

        # q_w(S_i, A_i): seleziona il Q-value dell'azione scelta per ogni transizione
        # gather: per ogni riga i, prende la colonna actions[i]
        q_sa = q_values.gather(1, actions.unsqueeze(1)).squeeze(1)  # (N,)

        # Target: R_i + γ * max_a q_w(S'_i, a) * (1 - done_i)
        with torch.no_grad():
            next_q_values = self.forward(next_states)  # (N, 3)
            max_next_q, _ = torch.max(next_q_values, dim=1)  # (N,)
            targets = rewards + gamma * max_next_q * (1 - dones)  # (N,)

        # Loss media sul batch: 1/N * Σ 1/2 * (target - q_sa)^2
...
def deepQlearning(env_name: str, num_episodes: int = 5000, render: bool = False,
    ...
    # Inizializza la rete, l'ottimizzatore e il replay buffer
    dqn = FreewayCNN()
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            # Forward pass: ottieni Q-values per tutte le azioni (no grad per la selezione)
            with torch.no_grad():
            ...
            # Preprocessing dello stato successivo
            preprocessed_next_obs = FreewayCNN.preprocess(next_obs)

            # --- Experience Replay: salva la transizione nel buffer ---
            # Tupla (S_t, A_t, R_{t+1}, S_{t+1}, done)
            replay_buffer.push(preprocessed_obs, action, reward,
                               preprocessed_next_obs, done)

            # --- Aggiorna la rete solo quando il buffer è pieno ---
            if replay_buffer.is_full():
                # Estrai l'intero buffer come batch
                states, actions_batch, rewards_batch, next_states, dones_batch = \
                    replay_buffer.sample_all()

                # Calcola la loss DQN sul batch:
                # L(w) = 1/N * Σ 1/2 * (R + γ * max_a q(S',a) - q(S, A))^2
                loss = dqn.compute_loss_batch(
                    states, actions_batch, rewards_batch,
                    next_states, dones_batch, gamma
                )
